            ...
            case ADC:
                switch (sub_command) {
                    ...
                    case CAPTURE_FOUR:
                        ...
                        buff0 = &ADCbuffer[0];
                        buff1 = &ADCbuffer[samples_to_fetch];
                        buff2 = &ADCbuffer[2 * samples_to_fetch];
                    ...
                    case CAPTURE_DMASPEED:
                        ...
                        AD1CON2bits.CHPS = 0;
                        _AD1IF = 0;
                        _AD1IE = 0; // Do not enable ADC interrupt. We're using DMA.
                        conversion_done = 1;
                        samples = samples_to_fetch; //assume it's all over already
                        if (value & 0x80) {
                            setADCMode(ADC_12BIT_DMA, value & 0x7F, 0);
                        }
                        else {
                            setADCMode(ADC_10BIT_DMA, value & 0x7F, 0);
                        }
                        ...
                        else if (sub_command == MULTIPOINT_CAPACITANCE) {
                            _TRISC0 = 0;
                            _LATC0 = 0;
                        } //Prepare 20K impedance voltage source, and connect it to 0V.
                    ...
                    case GET_CAPTURE_CHANNEL:
                        //disable_input_capture();
                        _LATC0 = 0;
                        _TRISC0 = 1; //set 20K CAP voltage source to high impedance mode.
                ...
                switch (sub_command) {
                    ...
                    case TIMING_MEASUREMENTS: //Using two input captures
                        ...
                        for (n = 0; n < ((cb)&0xF); n++) {
                            sendInt(IC1BUF);
                            sendInt(IC2BUF);
                        } //send data from PIN 1
                        for (n = 0; n < ((cb >> 4)&0xF); n++) {
                            sendInt(IC3BUF);
                            sendInt(IC4BUF);
                        }//send data from PIN 2
                        _IC3IF = 0;
                        _IC1IF = 0;
                        sendInt(IC2TMR);
                        disable_input_capture();
                    ...
                    case START_ALTERNATE_ONE_CHAN_LA: //Using input capture
                        ...
                        if (location & 7) {
                            ...
                            _IC4IF = 0;
                            _IC4IP = 7;
                            _IC4IE = 1; //enable input capture interrupt. highest priority
                        } else {
                    ...
                    case START_FOUR_CHAN_LA: //Using input capture
                        ...
                        if (value & 1) { //[7,6,5,4-B4,3-B6,2-B5,1-edge(rise/fall),0-enable]
                            ...
                            } //enable level change interrupt on B10(ID1)
                            if (value & 8) {
                                _CNIEB11 = 1;
                                DIGITAL_TRIGGER_CHANNEL |= 2;
                            } //enable level change interrupt on B11(ID2)
                            if (value & 16) {
                                _CNIEB12 = 1;
                                DIGITAL_TRIGGER_CHANNEL |= 4;
                            }//enable level change interrupt on B12(ID3)
                            _CNIF = 0;
                            _CNIE = 1;
                        } else {
                    ...
                    case FETCH_LONG_DMA_DATA: //Using input capture
                        ...
                        value = getChar(); //channel number
                        for (n = 0; n < lsb; n++) {
                            sendInt(ADCbuffer[n + 2 * value * (BUFFER_SIZE / 4)]);
                            sendInt(ADCbuffer[n + (2 * value + 1)*(BUFFER_SIZE / 4)]);
                        }//{sendInt(BufferA1[n]);sendInt(BufferA2[n]);}
                        LEDPIN = 1;
                        break;
                    ...
                    case FETCH_INT_DMA_DATA: //Using input capture
                        ...
                        value = getChar(); //channel number
                        for (n = 0; n < lsb; n++) {
                            sendInt(ADCbuffer[n + value * (BUFFER_SIZE / 4)]);
                        }//{sendInt(BufferA1[n]);sendInt(BufferA2[n]);}
                        LEDPIN = 1;
                        break;
            ...
            case COMMON:
                switch (sub_command) {
                    ...
                    case GET_FREQUENCY: //Using input capture
                        ...
                        while ((IC2TMR < lsb) && (!_IC1IF));
                        _IC1IF = 0;
                        RPINR7bits.IC1R = 0; //disconnect
                    ...
                    case SETRGB:
                        ...
                        _GIE = 0;
                        CSOUT1 = 0;
                        Delay_us(50); //reset LED
                        for (location = 0; location < value; location++) {
                            cb = data[location];
                            for (ca = 0; ca < 8; ca++) {
                                if (cb & 0x80) {
                                    CSOUT1 = 1;
                                    __asm__ volatile ("repeat #45");
                                    __asm__ volatile ("nop");
                                    CSOUT1 = 0;
                                    __asm__ volatile ("repeat #38");
                                    __asm__ volatile ("nop");
                                } else {
                                    CSOUT1 = 1;
                                    __asm__ volatile ("repeat #22");
                                    __asm__ volatile ("nop");
                                    CSOUT1 = 0;
                                    __asm__ volatile ("repeat #51");
                                    __asm__ volatile ("nop");
                                }
                                cb = cb << 1;
                            }
                ...
                switch (sub_command) {
                    ...
                    case NRF_TRANSACTION:
                        ...
                        if (ReadStatus() & 0x40) {
                            ...
                            logit("Stray Packet[0],");
                        } //In case a stray packet is found on pipe 0, flush it!
                        ...
                        while ((ReadStatus() & 0x30) == 0) {
                            Delay_us(100);
                        } //Wait for ACK / MAX_restransmits
                        asm("CLRWDT");
                        value = ReadStatus();
